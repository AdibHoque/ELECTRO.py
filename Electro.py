...
async def flipcoin(ctx):
    ...
    if pick == 'Heads':
    	heads=discord.Embed(color=color, description='{ctx.message.author.name} flipped Heads!')
    ...
    if pick == 'Tails':
    	tails=discord.Embed(color=color, description='{ctx.message.author.name} flipped Heads!')
...
async def define(ctx, *, msg:str=None):
    ...
    if msg is None:
        ...
        if len(response["list"]) == 0:
            return await client.say("No defination found!")
...
async def rps(ctx, *, message=None):
    ...
    if message.lower() != ans[0] and message.lower() != ans[1] and message.lower() != ans[2] :
        ...
        if message.lower()  == "rock" and pick == "paper":
            ...
            embed.add_field(name = "ELECTRO vs {}".format(ctx.message.author.name), value = "**Electro picked:** ✊{}\n**{} picked:** 🤚{}\n**Result: {} Wins!** Tie".format(pick, ctx.message.author.name, message, ctx.message.author.name))
            await bot.say(embed=embed)
        elif message.lower()  == "paper" and pick == "scissors":
            embed.add_field(name = "ELECTRO vs {}".format(ctx.message.author.name), value = "**Electro picked:** ✌{}\n**{} picked:** 🤚{}\n**Result: ELECTRO Wins!** Tie".format(pick, ctx.message.author.name, message))
            await bot.say(embed=embed)
