...
@bot.command(pass_context=True)
async def gay_(ctx, user: discord.Member = None):
	if user is None:
		score = random.randint(0, 100)
		r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
		embed = discord.Embed(title=f"Gayrate machine", description=f"{ctx.message.author} is **{score}%** gay :rainbow:", color = discord.Color((r << 16) + (g << 8) + b))
...
async def on_message(message):
    ...
    if message.server is None and message.author != bot.user:
    	 ...
    	 embed.timestamp = datetime.datetime.utcnow()
    	 embed.set_footer(text ='ELECTRO-MAIL', icon_url='https://cdn.discordapp.com/avatars/510491243155816449/b7e9c6f87f13a10c3da7bd681e3231ce.webp?size=1024')
    	 await bot.send_message(channel, embed=embed)    	 
    ...
    if not message.author.bot:
      ...
      for channel in message.author.server.channels:
        if channel.name == '📡electro-logs':
          ...
          embed.add_field(name = 'Channel:',value ='{}'.format(message.channel),inline = False)
          embed.timestamp = datetime.datetime.utcnow()
...
async def captcha(ctx, user:discord.Member):
	if user is None:
		url = f"https://nekobot.xyz/api/imagegen?type=captcha&image={ctx.message.author.avatar_url}&username={ctx.message.author.name}"
		async with aiohttp.ClientSession() as cs:
			async with cs.get(url) as r:
				res = await r.json()
				r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
		...
		url = f"https://nekobot.xyz/api/imagegen?type=captcha&image={user.avatar_url}&username={user.name}"
		async with aiohttp.ClientSession() as cs:
			async with cs.get(url) as r:
				res = await r.json()
				r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
