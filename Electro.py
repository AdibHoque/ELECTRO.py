...
async def on_command_error(error, ctx):
	await bot.say("There was a error executing this command or there isn't such command, please type `e!help` for a list of commands!")
...
async def servers(ctx):
  ...
  await bot.print(join(server.name for server in servers))						
...
@bot.command()
async def ownerinfo():
    await bot.say("__**THIS BOT WAS CREATED BY ADIB HOQUE**__           **DISCORD** - `@ADIB HOQUE#6969`  **YOUTUBE** - YouTube.com/AdibHoque")	
...
async def kick(ctx, user:discord.Member):
    ...
    if user.server_permissions.kick_members:
      ...
      return
    	...
    	await bot.delete_message(ctx.message)
    	await bot.send_message(user, 'You have been kicked out from {}'.format(ctx.message.server.name))
    	await bot.kick(user)
...
async def ban(ctx,user:discord.Member=None):
    ...
    if user.server_permissions.ban_members:
      ...
      return
    	...
    	await bot.send_message(user, 'You have banned from {ctx.message.server.name}')
    	await bot.ban(user)
...
async def help2(ctx):
    ...
    embed.add_field(name = 'Role',value ='Gives or removes role from mentioned user!**USAGE:**``e!role @user @role``',inline = False)
    ...
    embed.add_field(name = 'English',value ='Softwarns mentioned user to talk in English!**USAGE:**``e!english @user``',inline = False) 
...
async def help3(ctx):
    ...
    embed.add_field(name = 'Howgay',value ='Checks gayrate of mentioned user!**USAGE:**``e!howgay @user``',inline = False)
    await bot.send_message(author ,embed=embed)
