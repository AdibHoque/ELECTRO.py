...
@bot.command()
async def server():
	await bot.say('**PLEASE JOIN ELECTRO SUPPORT SERVER FROM THE LINK BELOW-** \nhttps://discord.gg/WpFAGyx**')
	
@bot.command()
async def support():
	await bot.say('**PLEASE JOIN ELECTRO SUPPORT SERVER FROM THE LINK BELOW-** https://discord.gg/WpFAGyx')	
	
@bot.command(pass_context=True)  
@commands.has_permissions(kick_members=True)     
async def kick(ctx, user:discord.Member, *, reason:str):
    if user is None:
      await bot.say('<:ElectroFail:527118399453528103>**Please mention a user to kick!**')
    if reason is None:
      await bot.say("<:ElectroFail:527118399453528103>**Please specify a reason for kicking!**")
...
async def ban(ctx, user:discord.Member, *, reason:str):
    if user is None:
      await bot.say('<:ElectroFail:527118399453528103>**Please mention a user to ban!**')
    if reason is None:
      await bot.say("<:ElectroFail:527118399453528103>**Please specify a reason for banning!**")
    if user.server_permissions.kick_members:
      await bot.say("<:ElectroFail:527118399453528103>**He is a Mod/Admin, I can't do that!**")
      return
    else:
      await bot.send_message(user, 'You were banned out from **{ctx.message.server.name}**, {reason}!')
      await bot.kick(user)
...
async def mentionable(ctx, *, role: discord.Role):
	if role.mentionable==True:
		...
		await bot.edit_role(server=ctx.message.server, role=role, mentionable=True)
		await bot.say('<:ElectroSucess:527118398753079317> That role is now mentionable!')
...
async def unmentionable(ctx, *, role: discord.Role):
	if role.mentionable==False:
		...
		await bot.edit_role(server=ctx.message.server, role=role, mentionable=True)
		await bot.say('<:ElectroSucess:527118398753079317> That role is now unmentionable!')		
...
async def rolecolor(ctx, role:discord.Role=None, value:str=None):
    ...
    if ctx.message.author.server_permissions.manage_roles == False:
        ...
        embed=discord.Embed(description="<:ElectroSucess:527118398753079317> {} role color has been changed!".format(role), color=colo)
        await bot.say(embed=embed) 
...
async def unbanall(ctx):
    if ctx.message.author.bot:
      ...
      channel = ctx.message.channel
      embed=discord.Embed(description="Unbanning {} Users!".format(len(ban_list)), color=0x00FFFF)
      await bot.send_message(channel, embed=embed)
...
async def on_message_edit(before, after):
    ...
    if before.author == bot.user:
      ...
      for channel in user.server.channels:
        if channel.name == '📡electro-logs':
            r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
            embed = discord.Embed(title = "MESSAGE EDITED", color = discord.Color((r << 16) + (g << 8) + b))
            ...
            embed.add_field(name = 'Channel:',value ='{}'.format(before.channel.mention),inline = False)
            embed.timestamp = datetime.datetime.utcnow()
...
async def on_reaction_add(reaction, user):
  for channel in user.server.channels:
    if channel.name == '📡electro-logs':
        logchannel = channel
        r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
        embed = discord.Embed(title = "REACTION ADDED", color = discord.Color((r << 16) + (g << 8) + b))
...
async def on_reaction_remove(reaction, user):
  for channel in user.server.channels:
    if channel.name == '📡electro-logs':
        logchannel = channel
        r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
        embed = discord.Embed(title = "REACTION REMOVED", color = discord.Color((r << 16) + (g << 8) + b))
        embed.add_field(name = 'Reaction by:',value ='{}'.format(user),inline = False)
        embed.add_field(name = 'Message:',value ='{}'.format(reaction.message.content),inline = False)
        embed.add_field(name = 'Channel:',value ='{}'.format(reaction.message.channel.mention),inline = False)
        embed.add_field(name = 'Emoji:',value ='{}'.format(reaction.emoji),inline = False)
    ...
    if not message.author.bot:
      ...
      for channel in message.author.server.channels:
        if channel.name == '📡electro-logs':
          logchannel = channel
          r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
          embed = discord.Embed(title = "MESSAGE DELETED", color = discord.Color((r << 16) + (g << 8) + b))
...
async def on_server_join(server):
	...
	embed.set_footer(text ='IM NOW IN {(len(servers))} GUILDS!')
	await bot.send_message(channel, embed=embed)		
