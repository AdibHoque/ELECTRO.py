...
helpm.add_field(name = '<:ElectroGeneralBadge:680783367247364097> GENERAL COMMANDS - (23)',value ='`ping`, `userinfo`, `serverinfo`, `ownerinfo`, `avatar`, `membercount`, `define`, `snipe`, `math`, `invite`, `upvote`, `pokemon`, `shinypokemon`, `pokefuse`, `8ball`, `electroav`, `brilliance`, `bravery`, `balance`, `motivation`, `revive`, `reversetext`, `chatbot`',inline = False)
...
async def status_task():
    while True:
        m = len(set(bot.get_all_members()))/1000
        await bot.change_presence(game=discord.Game(name='Help → e!help | {}K Users'.format(m)))
        await asyncio.sleep(12)
        await bot.change_presence(game=discord.Game(name='Help → e!help | '+str(len(bot.servers))+' Guilds'))
        await asyncio.sleep(12)
...
async def on_ready():
    print('ONLINE')
    print(len(set(bot.get_all_members()))/1000)
...
async def invite():
	...
	embed.add_field(name = 'Invite URL',value ='[https://invite.electro.xyz](https://discordapp.com/oauth2/authorize?client_id=629323586930212884&permissions=2146827775&redirect_uri=https%3A%2F%2Fdiscord.gg%2FdAggRh9&response_type=code&scope=guilds.join%20bot) or Direct Link https://discord.com/api/oauth2/authorize?client_id=629323586930212884&permissions=2146827775&scope=bot',inline = False)
	embed.add_field(name = 'Support Server',value ='[https://discord.gg/electro](https://discord.gg/dAggRh9)',inline = False)
	...
	await bot.say('https://discord.gg/dAggRh9', embed=embed)
...
async def rolecolor(ctx, role:discord.Role=None, value:str=None):
    ...
    if ctx.message.author.server_permissions.manage_roles == False:
        ...
        embed=discord.Embed(description="<a:ElectroCheck:709464171825201315> {} ROLE COLOR HAS BEEN CHANGED!".format(role.mention), color=0xFFBF00)
        await bot.say(embed=embed) 
...
#        embed = discord.Embed(color = 0xFFBF00, description = "Electrify & Boost Up Your Server With The Help Of ELECTRO.\nELECTRO Has 100+ Commands Which Features Moderation, Administration, Image Manipulation, Roleplay, Welcomer, Music, NSFW & Much More At Your Service!\n[Invite Now](https://discordapp.com/oauth2/authorize?client_id=629323586930212884&permissions=2146827775&redirect_uri=https%3A%2F%2Fdiscord.gg%2FdAggRh9&response_type=code&scope=guilds.join%20bot) or Join Support Server To Test It!")
...
async def on_server_join(server):
        ...
        embed.add_field(name = 'Server Owner:',value ='{}'.format(server.owner),inline = False)
