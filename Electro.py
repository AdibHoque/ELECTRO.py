...
@bot.command(pass_context = True)
async def embed(ctx, *, msg: str=None, msg2: str=None):
    channel = ctx.message.channel
    if msg is None or msg2 is None:
        await client.say('Use it like this ``e!embed [title] [despcription]``')
        return
    else:
        if member.server_permissions.administrator == False:
            await client.say('**Your role must have admin permission to use this command!**')
            return
        else:
            r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
