...
async def on_message(message):
    ...
    if '<@488353416599306270>' in message.content:
    	emoji = get(bot.get_all_emojis(), id='527117976772804639')
    ...
    if message.server is None and message.author != bot.user:
    	 ...
    	 embed.timestamp = datetime.datetime.utcnow()
    	 embed.set_footer(text ='ELECTRO', icon_url='https://cdn.discordapp.com/avatars/510491243155816449/b7e9c6f87f13a10c3da7bd681e3231ce.webp?size=1024')
    	 await bot.send_message(channel, embed=embed)    	 
    ...
    if not message.author.bot:
      ...
      for channel in message.author.server.channels:
        if channel.name == '📡electro-logs':
          ...
          embed.add_field(name = 'User: **{0}**'.format(user.name),value ='User ID: **{}**'.format(user.id),inline = False)
