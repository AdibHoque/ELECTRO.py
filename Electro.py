...
async def on_member_join(member):
    for channel in member.server.channels:
        ...
        if channel.name == '❓joins':
            await bot.say('**{member.name}** Welcome to **{member.server.name}** Hope you will be active here!')
            await bot.say('**We now have** 👨‍👩‍👦‍👦`{}` **members!**'.format(str(member.server.member_count)))

@bot.event
async def on_member_remove(member):
    for channel in member.server.channels:
        if channel.name == '❓leaves':
            await bot.say('**{member.name}** Just left **{member.server.name}!** Hope you will come back soon!')
            await bot.say('**We now have** 👨‍👩‍👦‍👦`{}` **members!**'.format(str(member.server.member_count)))
...
@bot.command()
async def ownerinfo():
    await bot.say("**__THIS BOT WAS CREATED BY ADIB HOQUE__**/n**DISCORD** - `@Adib Hoque#5782`/n**YOUTUBE** - YouTube.com/AdibHoque")	
