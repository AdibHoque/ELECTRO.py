...
helpm.add_field(name = '<:ElectroGeneralBadge:680783367247364097> GENERAL COMMANDS - (12)',value ='`ping`,`userinfo`,`serverinfo`,`ownerinfo`,`avatar`,`membercount`,`invite`,`upvote`,`pokemon`,`shinypokemon`,`pokefuse`,`8ball`',inline = False)
...
helpm.add_field(name = '<:ElectroFunBadge:680783413065941002> FUN COMMANDS - (20)',value ='`meme`,`pat`,`love`,`slap`, `kiss`, `hug`, `cuddle`, `spank`, `tweet`, `rolldice`, `flipcoin`, `howgay`, `whowouldwin`, `captcha`,`magik`,`deepfry`,`iphonex`,`threats`,`clyde`,`trash`',inline = False)
...
help1.add_field(name = 'Userinfo',value ='Shows info about mentioned user!\n**USAGE:**``e!info @user``',inline = False)
...
help2.add_field(name = 'DM',value ='Make ELECTRO DM mentioned user anything you want!\n**USAGE:**``e!dm @user [your text]``',inline = False) 
...
help2.add_field(name = 'SetupLogs',value ='Creates a log channel where electro posts some audit ligs!\n**USAGE:**``e!setuplogs``',inline = False)
...
help3.add_field(name = 'Virgin',value ='ELECTRO checks virginity of mentioned user!\n**USAGE:**``e!virgin @user``',inline = False)
...
async def phcomment(ctx, *, msg:str):
	url = f"https://nekobot.xyz/api/imagegen?type=phcomment&image={ctx.author.avatar_url}&text={msg}&username={ctx.author.name}" 
	async with aiohttp.ClientSession() as cs:
		async with cs.get(url) as r:
			res = await r.json()
			r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
